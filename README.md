...
## Independent sample verification tool

Use `verify_gene_order_output.py` to independently check a **small, user-defined sample**
of records from the main output file.

It verifies, for sampled positions:

1. The position maps to a real CDS (`locus_tag`) in the genome GFF order
2. The reported gene-family token matches the family assignment for that locus in the
   pangenome presence-absence table

The tool produces a report with:

- run settings (sample size, seed, unknown token, paths)
- summary counts (checked positions, matches, mismatches)
- per-record details (length checks + sampled position checks)

### Verifier usage

```bash
python verify_gene_order_output.py \
    --presence-absence gene_presence_absence.csv \
    --gff-dir /path/to/gff/files \
    --result-file gene_order.txt \
    --sample-records 5 \
    --sample-genes 20 \
    --seed 1 \
...
If your main output was generated with `--strand mark`, pass:

```bash
--strip-strand-mark
```

so trailing `+` / `-` is removed before comparison.

### Verifier options

| Flag | Short | Default | Description |
|------|-------|---------|-------------|
| `--presence-absence` | `-p` | *(required)* | Path to Roary presence-absence CSV |
| `--gff-dir` | `-g` | *(required)* | Directory containing genome GFF files |
| `--result-file` | `-r` | *(required)* | Output file generated by `gene_order_from_pangenome.py` |
| `--report` | `-o` | stdout | Write verification report to file |
| `--sample-records` | | `5` | Number of output records (headers) to sample |
| `--sample-genes` | | `20` | Number of gene positions to sample per selected record |
| `--seed` | | `1` | Random seed for reproducible sampling |
| `--meta-cols` | `-m` | `14` | Number of metadata columns before genome columns in CSV |
| `--unknown` | `-u` | `*` | Unknown token expected in output |
| `--strip-strand-mark` | | *(off)* | Remove trailing `+`/`-` from output tokens before comparison |
